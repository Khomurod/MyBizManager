...
function renderHistory() {
    ...
    list.innerHTML = "";
    // Newest first: sort by period, then by the id's timestamp prefix, so
    // history no longer depends on sheet row order.
    [...app.transactions]
        .sort((a, b) => comparePeriods(recordPeriod(a), recordPeriod(b)) ||
                        (Number(getTxBaseId(a.id)) || 0) - (Number(getTxBaseId(b.id)) || 0))
        .reverse()
        .forEach(t => {
        ...
        div.innerHTML = `
            <div class="flex items-center gap-3">
                ...
                <div><p class="font-bold text-slate-700 text-sm">${t.tenant}</p><p class="text-[10px] text-slate-400">${t.date} • ${periodLabel(recordPeriod(t))}</p></div>
            </div>
            <div class="flex items-center gap-3">
