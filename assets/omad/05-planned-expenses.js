...
function normalizeTemplateExpense(rawExpense) {
    ...
    return {
        ...
        // A legacy month name is kept verbatim rather than given an invented
        // year; the backend migration assigns one under operator control.
        month: isPeriod(expense.period) ? expense.period
             : (isPeriod(expense.month) ? expense.month
             : (String(expense.month || "").trim() || currentPeriod())),
...
function addTemplateExpense() {
    ...
    if(!isPeriod(month)) return alert("Davrni tanlang");
